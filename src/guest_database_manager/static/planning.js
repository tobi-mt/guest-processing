  ...
  visibleRecommendations.forEach((episode, index) => {
    ...
    card.innerHTML = `
      ...
      <details class="recommendation-evidence">
        ...
        ${episode.production_readiness_forecast ? `<div class="operations-preview"><strong class="insight-label">Production forecast</strong><p>${escapeHtml(`${episode.production_readiness_forecast.feasible ? "Feasible" : "At risk"} · estimated ready ${episode.production_readiness_forecast.estimated_ready_by} · ${episode.production_readiness_forecast.buffer_days} day buffer`)}</p><p>${escapeHtml(episode.production_readiness_forecast.assumption || "")}</p></div>` : ""}
        ${episode.calendar_capacity ? `<div class="operations-preview"><strong class="insight-label">Calendar capacity</strong><p>${escapeHtml(`${episode.calendar_capacity.cadence} · ${episode.calendar_capacity.weeks_until_slot} weeks until slot · ${episode.calendar_capacity.reserved_release_count} future slots reserved`)}</p><p>${escapeHtml(episode.calendar_capacity.constraint || "")}</p></div>` : ""}
        ${episode.audience_fatigue ? `<div class="operations-preview"><strong class="insight-label">Audience fatigue · ${escapeHtml(episode.audience_fatigue.level)}</strong><p>${escapeHtml(`${episode.audience_fatigue.message} Category hits: ${episode.audience_fatigue.category_hits}; topic-cluster hits: ${episode.audience_fatigue.topic_cluster_hits}.`)}</p></div>` : ""}
        ${episode.time_sensitive_event_alignment ? `<div class="operations-preview"><strong class="insight-label">Time-sensitive guest event</strong><p>${escapeHtml(`${episode.time_sensitive_event_alignment.title} · ${episode.time_sensitive_event_alignment.date}`)}</p></div>` : ""}
        ${episode.recommendation_history?.length ? `<div class="operations-preview"><strong class="insight-label">Recommendation history</strong><ol>${episode.recommendation_history.slice(0, 5).map((item) => `<li>${escapeHtml(String(item.event_type || "decision").replaceAll("_", " "))} · ${escapeHtml(item.occurred_at || "")} · ${escapeHtml(item.actor || "system")}</li>`).join("")}</ol></div>` : ""}
        ${renderPromoReadiness(episode.promotion_readiness)}
        ${renderGuestResearchCopilot(episode.guest_research)}
        ${renderCopyAssist(episode.copy_assist)}
        <div class="context-links">
